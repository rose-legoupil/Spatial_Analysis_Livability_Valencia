...
subtitle: "Reproducible analysis" # only for html output 
date: "`r Sys.Date()`"
output:
...
The two metrics were then used to calculate a composite indicator of social diversity displayed in the sample calculation in *Fig.8*. Both indicators where normalized then the social diversity index for each district was calculated by adding both normalized indicators (equal weighting was used as it is assumed that both indicators have equal importance). 
...
To conclude this investigation, a final graph encompassing all the variables explored was generated as a bubble chart shown on *Fig.17* which is useful to make compasion when a lot of inductors are used. Livability and environmental quality was calculated as a single inductor by normalizing their respective components and aggregating them into composite scores.        
